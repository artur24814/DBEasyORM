...
def test_filtering_models_with_startswith_operator(testing_db):
    ...
    assert CustomeTestModel.query_creator.filter(name__startswith="Jo").sql == \
        "SELECT CUSTOMETESTMODEL.* FROM CUSTOMETESTMODEL WHERE name LIKE 'Jo%'"
    queryset_name = CustomeTestModel.query_creator.filter(name__startswith="Jo").execute()
